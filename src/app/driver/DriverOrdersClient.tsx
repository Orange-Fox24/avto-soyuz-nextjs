...
const MapComponent = dynamic(() => import("./RouteMap"), { ssr: false });
...
export default function DriverOrdersClient({ orders }: { orders: Order[] }) {
  ...
  const handleProblem = async () => {
    ...
    const problem = prompt("Опишите проблему (поломка, ДТП, опоздание):");
    if (!problem) return;

    try {
      await fetch("/api/driver/problem", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ orderId: selectedOrder.id, problem }),
      });
  ...
  return (
    <>
          ...
          <div key={order.id} className={styles.orderCard}>
            ...
            <div className={styles.actions}>
              ...
              <button
                ...
                onClick={() => { setSelectedOrder(order); setShowProblem(false); }}
              >
                🗺 Маршрут
              </button>
              <button
                ...
                onClick={() => { setSelectedOrder(order); setShowProblem(true); }}
              >
                ⚠ Проблема
              </button>
            </div>
      ...
      {/* Модальное окно: карта или проблема */}
      {selectedOrder && !showProblem && (
        <div style={{
          ...
          <div style={{
            backgroundColor: "#fff", borderRadius: "24px", padding: "1.5rem",
            ...
            <MapComponent fromCity={selectedOrder.fromCity} toCity={selectedOrder.toCity} />
            <button
              onClick={() => setSelectedOrder(null)}
              style={{
                marginTop: "1rem", backgroundColor: "#ffa20c", color: "#fff",
                fontFamily: '"Wadik Bold", "Wadik", Arial, Helvetica, sans-serif',
                fontWeight: 700, padding: "0.75rem 1.5rem", border: "none",
      ...
      {selectedOrder && showProblem && (
        <div style={{
          ...
          <div style={{
            ...
            <p style={{ fontFamily: "Actay, Arial, sans-serif", fontSize: "0.9rem", color: "#6b7280", marginBottom: "1rem" }}>
              Заказ: {selectedOrder.fromCity} → {selectedOrder.toCity}
            </p>
            <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
              {["Поломка транспорта", "ДТП", "Опоздание на погрузку", "Проблемы с грузом", "Другое"].map((p) => (
                <button
                  key={p}
                  onClick={() => {
                    if (window.confirm(`Подтвердить: "${p}"?`)) {
                      fetch("/api/driver/problem", {
                        method: "POST",
                        headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({ orderId: selectedOrder.id, problem: p }),
                      }).then(() => {
                        alert("Проблема зафиксирована. Менеджер уведомлён.");
                        setShowProblem(false);
                  ...
                  style={{
                    padding: "0.75rem", backgroundColor: "#fef2f2", color: "#991b1b",
            ...
            <button
              onClick={() => setShowProblem(false)}
              style={{
                marginTop: "1rem", backgroundColor: "#6b7280", color: "#fff",
                fontFamily: '"Wadik Bold", "Wadik", Arial, Helvetica, sans-serif',
                fontWeight: 700, padding: "0.75rem 1.5rem", border: "none",
                borderRadius: "12px", cursor: "pointer", width: "100%",
              }}
